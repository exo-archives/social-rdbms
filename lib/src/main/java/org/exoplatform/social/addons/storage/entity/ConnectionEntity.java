...
@NamedQueries({
        ...
        @NamedQuery(name = "SocConnection.getSenderIdConnectionsByStatus",
                query = "SELECT connection.sender.id from SocConnection connection where connection.receiver.id = :identityId AND connection.status = :connStatus"),
        @NamedQuery(name = "SocConnection.getReceiverIdConnectionsByStatus",
                query = "SELECT connection.receiver.id from SocConnection connection where connection.sender.id = :identityId AND connection.status = :connStatus"),
        ...
        @NamedQuery(name = "SocConnection.countReceiverBySenderAndStatus", query = "SELECT count(c.receiver.id) FROM SocConnection c WHERE c.sender.id = :senderId AND c.status in (:status)"),
        @NamedQuery(name = "SocConnection.countSenderByReceiverAndStatus", query = "SELECT count(c.sender.id) FROM SocConnection c WHERE c.receiver.id = :receiverId AND c.status in (:status)"),
