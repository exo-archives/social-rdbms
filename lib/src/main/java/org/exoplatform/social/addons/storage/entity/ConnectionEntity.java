...
@NamedQueries({
        ...
        @NamedQuery(name = "SocConnection.getConnections",
                query = "SELECT connection.sender.id as sender, connection.receiver.id  as receiver from SocConnection connection where (connection.sender.id = :identityId or connection.receiver.id = :identityId) AND connection.status = :status"),
        @NamedQuery(name = "SocConnection.getConnectionsWithStatus", query = "SELECT c FROM SocConnection c WHERE (c.sender.id = :identityId OR c.receiver.id = :identityId) AND c.status = :status"),
